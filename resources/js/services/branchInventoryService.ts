...
export class BranchInventoryService {
    ...
    static async getBranchInventory(branchId: number): Promise<BranchInventoryItem[]> {
        try {
            console.log('=== GET BRANCH INVENTORY DEBUG ===');
            console.log('Getting branch inventory from medicine_stock_in for branch:', branchId);
            ...
            console.log('Stock in data fetched:', stockInData?.length, 'records');
            ...
            console.log('Stock out data fetched:', stockOutData?.length, 'records');
            ...
            if (deletedError) {
                console.error('Error fetching deleted medicine data:', deletedError);
                throw deletedError;
            }

            console.log('Deleted data fetched:', deletedData?.length, 'records');

            // Create a map of dispensed quantities by medicine_stock_in_id
            const dispensedMap: { [key: number]: number } = {};
            stockOutData?.forEach(record => {
                if (dispensedMap[record.medicine_stock_in_id]) {
                    dispensedMap[record.medicine_stock_in_id] += record.quantity_dispensed;
                } else {
                    dispensedMap[record.medicine_stock_in_id] = record.quantity_dispensed;
                }
            });

            // Create a map of deleted quantities by medicine_stock_in_id
            const deletedMap: { [key: number]: number } = {};
            deletedData?.forEach(record => {
                if (deletedMap[record.medicine_stock_in_id]) {
                    deletedMap[record.medicine_stock_in_id] += record.quantity;
                } else {
                    deletedMap[record.medicine_stock_in_id] = record.quantity;
                }
            });

            console.log('Processing stock records for available quantities...');

            // Create separate entries for each stock record (no grouping)
            const result: BranchInventoryItem[] = [];

            stockInData?.forEach(record => {
                const stockInId = record.medicine_stock_in_id;
                const totalDispensed = dispensedMap[stockInId] || 0;
                const totalDeleted = deletedMap[stockInId] || 0;
                const availableQuantity = Math.max(0, record.quantity - totalDispensed - totalDeleted);
                
                console.log(`Stock ID ${stockInId}: Original=${record.quantity}, Dispensed=${totalDispensed}, Deleted=${totalDeleted}, Available=${availableQuantity}`);
                
                // Only include records with available quantity > 0
                if (availableQuantity > 0) {
                    const medicineData = Array.isArray(record.medicine) ? record.medicine[0] : record.medicine;
                    result.push({
                        ...
                        // Store the stock_in_id for reference (helpful for operations)
                        medicine_stock_in_id: stockInId
                    });
                }
            });
            
            console.log('Final result:', result.length, 'available stock records');
            console.log('=== GET BRANCH INVENTORY SUCCESS ===');
            return result;
    ...
    static async createMedicine(
        ...
        try {
            ...
            const { data: existingMedicine, error: checkError } = await supabase
                ...
                .eq('medicine_category', medicineCategory)
                .single();

            console.log('Existing medicine check:', { existingMedicine, checkError });

            if (existingMedicine) {
                console.log('Medicine already exists, returning:', existingMedicine);
            ...
            console.log('Successfully created new medicine:', newMedicine);
            return newMedicine;
        } catch (error) {
    ...
    static async addMedicineStockIn(
        ...
        try {
            ...
            if (dateReceived) {
                stockInData.date_received = dateReceived;
            }

            if (expirationDate) {
                stockInData.expiration_date = expirationDate;
            }
            ...
            console.log('Successfully added medicine stock in:', data);
            return data;
        } catch (error) {
    ...
    // Dispense medicine using the medicine_stock_out table
    static async dispenseMedicineStockOut(
        medicineStockInId: number,
        branchId: number,
        userId: number,
        quantityDispensed: number
    ): Promise<MedicineStockOut | null> {
        try {
            console.log('Dispensing medicine:', { medicineStockInId, branchId, userId, quantityDispensed });

            // First check if the stock in record exists and has enough quantity
            const { data: stockInRecord, error: stockInError } = await supabase
                .from('medicine_stock_in')
                .select('*')
                .eq('medicine_stock_in_id', medicineStockInId)
                .single();

            if (stockInError || !stockInRecord) {
                throw new Error('Stock in record not found');
            }

            // Check if there's enough quantity available (considering previous dispensals)
            const { data: previousDispensals, error: dispensalError } = await supabase
                .from('medicine_stock_out')
                .select('quantity_dispensed')
                .eq('medicine_stock_in_id', medicineStockInId);

            if (dispensalError) {
                console.error('Error checking previous dispensals:', dispensalError);
                throw dispensalError;
            }

            const totalDispensed = previousDispensals?.reduce((sum, record) => sum + record.quantity_dispensed, 0) || 0;
            const availableQuantity = stockInRecord.quantity - totalDispensed;

            if (availableQuantity < quantityDispensed) {
                throw new Error(`Insufficient stock. Available: ${availableQuantity}, Requested: ${quantityDispensed}`);
            }
            ...
            console.log('Successfully dispensed medicine:', data);
            return data;
        } catch (error) {
            console.error('Error in dispenseMedicineStockOut:', error);
            return null;
        }
    }
    ...
    static async getAvailableStockInRecords(medicineId: number, branchId: number): Promise<{
        ...
        try {
            ...
            const { data: stockInData, error: stockInError } = await supabase
                ...
                .eq('medicine_id', medicineId)
                .eq('branch_id', branchId)
                .order('date_received', { ascending: true }); // FIFO - oldest first
            ...
            console.error('Error getting available stock in records:', error);
            return [];
        }
    }
    ...
    // Dispense medicine with history logging
    static async dispenseMedicine(
        ...
        try {
            ...
            if (stockOutError) {
                console.error('❌ Error dispensing medicine:', stockOutError);
                return false;
            }

            console.log('✅ Medicine dispensed successfully');

            // Log to history_log table
            if (stockOutData?.medicine_stock_out_id) {
                const medicineName = stockOutData.medicine_stock_in?.medicine?.[0]?.medicine_name || 
                                   (stockOutData.medicine_stock_in?.medicine as any)?.medicine_name ||
                                   'Unknown Medicine';

                const historyLogged = await HistoryLogService.logMedicineDispense(
                    userId,
                    stockOutData.medicine_stock_out_id,
                    `Dispensed ${quantityDispensed} units of ${medicineName} to ${patientName} (Prescribed by: ${prescribedBy})`
                );
                
                if (historyLogged) {
                    console.log('✅ Medicine dispensing logged to history_log');
                } else {
                    console.warn('⚠️ Failed to log medicine dispensing to history_log');
                }
            }
